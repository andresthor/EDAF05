...
public class Graph {
	...
	private MultiSet<Edge>[] neighbors;
	private Edge[] edges;
	private int lastEdge;
	
	public Graph(int V) {
		if (V < 0) throw new IllegalArgumentException("Vertices must be nonnegative");
		this.V = V;
		this.E = 0;
		edges = new Edge[V];
		lastEdge = 0;
		neighbors = (MultiSet<Edge>[]) new MultiSet[V];
		for (int i = 0; i < V; i++) {
			neighbors[i] = new MultiSet<Edge>();
		}
	}
	...
	private void validateVertex(Vertex v) {
		if (v.index() < 0 || v.index() >= V)
			throw new IndexOutOfBoundsException("Vertex index is not within size of graph");
	}
	...
	public void addEdge(Edge e) {
		...
		validateVertex(v);
		validateVertex(w);
		neighbors[v.index()].add(e);
		neighbors[w.index()].add(e);
		edges[lastEdge] = e;
		E++;
	}
	
	public Iterable<Edge> neighbors(int i) {
		// validateVertex(i);
		return neighbors[i];
	}
